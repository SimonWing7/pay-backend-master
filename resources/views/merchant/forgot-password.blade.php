...
<head>
    ...
    <title>Forgot Password - Edfundo Pay</title>
...
<body style="background-color: #f5f6fa; min-height: 100vh; display: flex; align-items: center; justify-content: center;">
    <div style="width: 100%; max-width: 440px; padding: 0 16px;">
        ...
        <div style="text-align: center; margin-bottom: 32px;">
            ...
            <h1 style="font-size: 22px; font-weight: 800; color: #000026; margin: 0;">Reset Password</h1>
            <p style="font-size: 14px; color: #9ca3af; margin-top: 4px;">Enter your email and we'll send a reset link</p>
        </div>
        ...
        <div style="background: white; border-radius: 16px; padding: 36px; box-shadow: 0 4px 24px rgba(0,0,38,0.08); border: 1px solid #eef0f5;">
            ...
            @if(session('status'))
                <div style="background: #f0fdf4; border: 1px solid #bbf7d0; color: #166534; padding: 12px 16px; border-radius: 8px; font-size: 14px; margin-bottom: 20px;">
                    {{ session('status') }}
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('merchant.password.forgot.post') }}">
                @csrf
        ...
        <p style="text-align: center; font-size: 13px; color: #9ca3af; margin-top: 20px;">
            <a href="{{ route('merchant.login') }}" style="color: #3d01bd; font-weight: 600; text-decoration: none;">← Back to sign in</a>
        </p>
